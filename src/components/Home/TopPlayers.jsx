...
const TopPlayers = () => {
    const categories = ['Senior', 'Junior', 'Juvenil', 'Infantil', 'Alevín'];
    const [selectedCategory, setSelectedCategory] = React.useState('Senior');
    const [showAll, setShowAll] = React.useState(false);

    // Filter and sort players by selected category
    const filteredPlayers = playerStats
        .filter(p => p.category === selectedCategory)
        .sort((a, b) => b.points - a.points);

    // Take top 10 or all players based on state
    const displayCount = showAll ? filteredPlayers.length : 10;
    const topPlayers = filteredPlayers.slice(0, displayCount);
        ...
        <section className="py-12 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
            <div className="container mx-auto px-4">
                ...
                <div className="flex flex-wrap justify-center gap-2 mb-10">
                    {categories.map((cat) => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-full text-sm font-bold transition-all duration-200 ${selectedCategory === cat
                                ? 'bg-halcones-blue text-white shadow-lg scale-105'
                ...
                {filteredPlayers.length > 0 ? (
                    <>
                        {/* Podium */}
                        <div className="flex justify-center items-end gap-2 md:gap-4 mb-12 max-w-3xl mx-auto">
                            {podium.map((player, index) => (
                                player && (
                                    <motion.div
                                        key={`${selectedCategory}-${player.name}`}
                                    ...
                                    >
                                        <div className="relative mb-2">
                                            <div className={`w-16 h-16 md:w-24 md:h-24 rounded-full border-4 overflow-hidden bg-white shadow-lg ${index === 1 ? 'border-yellow-400 w-20 h-20 md:w-32 md:h-32 z-10' :
                                            ...
                                            <div className={`absolute -bottom-2 md:-bottom-3 left-1/2 transform -translate-x-1/2 w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center font-bold text-white shadow-md text-xs md:text-base ${index === 1 ? 'bg-yellow-500' :
                                        ...
                                        <div className="text-center mb-2 px-1">
                                            <h3 className="font-bold text-gray-900 dark:text-white text-[10px] md:text-base line-clamp-1">{player.name}</h3>
                                        </div>
                                        ...
                                        <div className={`w-full rounded-t-lg bg-gradient-to-b ${podiumColors[index]} flex flex-col items-center justify-center shadow-lg ${podiumHeight[index]}`}>
                                            <span className="text-xl md:text-3xl font-bold">{player.points}</span>
                                            <span className="text-[8px] md:text-sm font-medium uppercase opacity-80">Puntos</span>
                                            <div className="flex gap-1 md:gap-2 mt-1 text-[8px] md:text-xs opacity-75">
                        ...
                        <div className="max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden">
                            {list.map((player, index) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-gray-500 dark:text-gray-400">{selectedCategory}</p>
                                    </div>
                            ...
                            {filteredPlayers.length > 10 && (
                                <div className="p-4 text-center border-t border-gray-100 dark:border-gray-700">
                                    ...
                                    >
                                        {showAll ? 'Ver menos' : `Ver todos los jugadores (${filteredPlayers.length})`}
                                    </button>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-xl shadow-md max-w-2xl mx-auto">
                        <p className="text-gray-500 dark:text-gray-400">No hay estadísticas disponibles para esta división.</p>
                    </div>
                )}
            </div>
        </section>
